...
try:
    ...
    from tools.utils.utils import get_case_message
except ImportError:
    ApiTestCase = None
    get_case_message = None
...
class JSONFile:
    ...
    def collect(self):
        ...
        try:
            ...
            # JSON解析错误，跳过该文件
            pytest.skip(f"Failed to parse JSON file {self.fspath}: {e}")
            return
        except Exception as e:
            pytest.skip(f"Failed to read JSON file {self.fspath}: {e}")
            return

        # 验证必要的字段存在
        if not self._validate_test_data():
            pytest.skip(f"Invalid test data format in {self.fspath}")
            return
...
class JSONCaseItem:
    ...
    def __init__(
        ...
        # 用于存储stash数据（pytest钩子间共享数据）
        self.stash: Dict[str, Any] = {}

    @staticmethod
    def from_parent(parent, name: str, case_data: Dict[str, Any], case_type: str = "API", case_id: str = "", case_name: str = ""):
        """
...
def pytest_collect_file(parent, file_path: Path):
    ...
    # 收集所有JSON文件
    if file_path.name.startswith("test") or file_path.name.endswith(".json"):
