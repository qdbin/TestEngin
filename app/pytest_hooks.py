...
ALLURE_ENABLED = os.environ.get("ALLURE_ENABLED", "false").lower() == "true"  # 默认关闭Allure
...
class AllureObserver:
    ...
    def _dict_to_html_table(self, data: Dict[str, Any]) -> str:
        ...
        for key, value in data.items():
            rows.append(f"<tr><td style='padding:4px;border:1px solid #ddd;'>{key}</td><td style='padding:4px;border:1px solid #ddd;'><pre>{value}</pre></td></tr>")
...
def pytest_configure(config):
    ...
    # 获取平台结果队列
    PYTEST_RESULT_QUEUE = config.getoption("--result-queue", default=None)
